...
namespace myslam
{
	namespace hesai
	{
		...
		PandarGeneral_Internal::PandarGeneral_Internal(std::string pcap_path, \
			...
			pcl_callback, int start_id, int end_id, std::string lidar_type, std::string lidar_correction_file, uint16_t start_angle, int pcl_type) {
			...
			process_num_ = 0;
			pcap_reader_ = new io::PcapReader(pcap_path, lidar_type, start_id, end_id);
		...
		int PandarGeneral_Internal::Start() {
			...
			lidar_process_thr_ = new boost::thread(boost::bind(&PandarGeneral_Internal::ProcessLiarPacket, this));
		...
		void PandarGeneral_Internal::EmitBackMessege(char chLaserNumber, boost::shared_ptr<PPointCloud> cld) {
			...
			pcl_callback_(cld, cld->points[0].tm); // the timestamp from first point cloud of cld
			if (pcl_type_) {
				for (int i = 0; i<chLaserNumber; i++) {
		...
		void PandarGeneral_Internal::FillPacket(const uint8_t *buf, const int len, double timestamp) {
			if (len != GPS_PACKET_SIZE) {
				//hesai_lidar::PandarPacket pkt;
				PandarRaw pkt;
				//memcpy((&pkt.data), &buf, len);
				pkt.rawdata = new uint8_t[len];
				memcpy(pkt.rawdata, buf, len);
				pkt.size = len;
				pkt.stamp = timestamp;
				//delete[]pkt.rawdata;
				PushLiDARData(pkt);
		...
		void PandarGeneral_Internal::ProcessLiarPacket() {
			...
			while (enable_lidar_process_thr_) {
				...
				if (m_PacketsBuffer1.size() == 0)
				{
					std::this_thread::sleep_for(std::chrono::milliseconds(50));
					continue;
				}
				process_lock.lock();
				//PandarRaw packet = *(m_PacketsBuffer.getIterCalc());
				//m_PacketsBuffer.moveIterCalc();
				PandarRaw packet = m_PacketsBuffer1.front();
				m_PacketsBuffer1.pop();
				process_lock.unlock();
				process_num_++;
				if (packet.size == PACKET_SIZE || packet.size == PACKET_SIZE + SEQ_NUM_SIZE) {
					Pandar40PPacket pkt;
					ret = ParseRawData(&pkt, packet.rawdata, packet.size);
					...
					ret = ParseL64Data(&pkt, packet.rawdata, packet.size);
					...
					ret = ParseL20Data(&pkt, packet.rawdata, packet.size);
					...
					ret = ParseQTData(&pkt, packet.rawdata, packet.size);
					if (ret != 0) {
						continue;
					}
					for (int i = 0; i < pkt.header.chBlockNumber; ++i) {
					...
					ret = ParseXTData(&pkt, packet.rawdata, packet.size);
					...
					for (int i = 0; i < pkt.header.chBlockNumber; ++i) {
						...
						timestampGap = pkt.timestamp - last_timestamp_ + 0.001;
						if (last_azimuth_ != pkt.blocks[i].azimuth && (azimuthGap/timestampGap) < MAX_AZIMUTH_DEGREE_NUM * 100 /* 6 degree*/) {
							...
							if ((last_azimuth_ > pkt.blocks[i].azimuth &&
								...
								(last_azimuth_ < start_angle_ &&
									start_angle_ <= pkt.blocks[i].azimuth)) {
								//cout << "gap: " << i << " " << pkt.blocks[i].azimuth << " " << last_azimuth_ << " " << azimuthGap << " " << timestampGap << " " << endl;
		...
		void PandarGeneral_Internal::PushLiDARData(PandarRaw packet) {
			process_lock.lock();
			m_PacketsBuffer1.push(packet);
			process_lock.unlock();
		}
		...
		void PandarGeneral_Internal::ProcessGps(const PandarGPS &gpsMsg) {
			...
			//if (gps_callback_) {
			 // gps_callback_(static_cast<double>(mktime(&t) + tz_second_),static_cast<std::string>("abc") );
			  //}
		}
		...
		void PandarGeneral_Internal::CalcPointXYZIT(Pandar40PPacket *pkt, int blockid,
			...
			for (int i = 0; i < LASER_COUNT; ++i) {
				...
				if ("realtime" == m_sTimestampType) {
					point.tm = m_dPktTimestamp;
				}
				else {
					point.tm = unix_second + \
			...
			for (int i = 0; i < chLaserNumber; ++i) {
				...
				if ("realtime" == m_sTimestampType) {
					...
					if (pkt->echo == 0x39) {
						// dual return, block 0&1 (2&3 , 4*5 ...)'s timestamp is the same.
						point.tm =
							point.tm - (static_cast<double>(block64OffsetDual_[blockid] +
								laser64Offset_[i]) /
								1000000.0f);
					}
					else {
			...
			for (int i = 0; i < chLaserNumber; ++i) {
				...
				if ("realtime" == m_sTimestampType) {
					...
					if (pkt->echo == 0x05) {
						...
						point.tm =
							point.tm + (static_cast<double>(blockQTOffsetDual_[blockid] +
								laserQTOffset_[i]) /
								1000000.0f);
					}
					else {
		...
		void PandarGeneral_Internal::CalcXTPointXYZIT(HS_LIDAR_XT_Packet *pkt, int blockid, \
			...
			double unix_second = \
				static_cast<double>(mktime(&tTm) + tz_second_);
			//cout << "process: " << tTm.tm_year << " " << tTm.tm_mon << " " << tTm.tm_mday << " " << tTm.tm_hour << " " << tTm.tm_min << " " << tTm.tm_sec << " " << unix_second << endl;    
			for (int i = 0; i < chLaserNumber; ++i) {
				/* for all the units in a block */
				...
				if ("realtime" == m_sTimestampType) {
					...
					if (pkt->echo == 0x3d) {
						...
						point.tm = point.tm + (static_cast<double>(blockXTOffsetTriple_[blockid] +
							laserXTOffset_[i]) /
							1000000.0f);
					}
					else if (pkt->echo == 0x39 || pkt->echo == 0x3b || pkt->echo == 0x3c) {
						
						point.tm = point.tm + (static_cast<double>(blockXTOffsetDual_[blockid] +
							laserXTOffset_[i]) /
							1000000.0f);
					}
					else {

						point.tm = point.tm + (static_cast<double>(blockXTOffsetSingle_[blockid] + laserXTOffset_[i]) / \
							1000000.0f);
					}
				}

				point.ring = hesai2velodyne(i);
				if (pcl_type_) {
					PointCloudList[i].push_back(point);
				}
				else {
