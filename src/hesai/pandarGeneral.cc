...
PandarGeneral::PandarGeneral(
    ...
    boost::function<void(double, std::string)> gps_callback, 
    boost::function<void(std::vector<float>)> conf_callback,uint16_t start_angle, int tz,
    int pcl_type, std::string lidar_type, std::string frame_id, std::string timestampType,
    std::string lidar_correction_file, std::string multicast_ip, bool coordinate_correction_flag, std::string cali_file) {
      // LOG_FUNC();
  internal_ =
      new PandarGeneral_Internal(device_ip, lidar_port, gps_port, pcl_callback,
                             gps_callback, conf_callback,start_angle, tz, pcl_type, lidar_type, frame_id, timestampType, lidar_correction_file, multicast_ip, coordinate_correction_flag, cali_file);
}
