...
#include "PandarScan.h"
#include "PandarPacket.h"
#include "io/pcap_parse.h"
#include <unistd.h>
#include <queue>
...
namespace myslam {
	...
	namespace hesai
	{
		...
		typedef struct PandarRaw_s PandarRaw;
		
		typedef std::array<PandarRaw, 36000> PktArray;
		typedef struct PacketsBuffer_s {
			...
			inline int push_back(PandarRaw pkt) {
				if (!m_startFlag) {
					*m_iterPush = pkt;
					m_startFlag = true;
					return 1;
...
//
		...
		public:
			...
			PandarGeneral_Internal(
				...
				pcl_callback, int start_id = 0, int end_id = INT_MAX, std::string lidar_type = "PandarXT_16", std::string lidar_correction_file = "", uint16_t start_angle = 0,  int pcl_type = 0);
			~PandarGeneral_Internal();
		...
		private:
			...
			void PushLiDARData(PandarRaw packet);
			...
			std::queue<PandarRaw> m_PacketsBuffer1;
			bool m_bCoordinateCorrectionFlag;
			std::string m_cali_file;
